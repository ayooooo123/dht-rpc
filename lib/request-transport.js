...
class RequestTransport {
  ...
  createRequest(
    ...
    if (session && session.destroyed) throw session.error

    const identity =
      candidate === null ? this._identity(to, false) : this._candidateIdentity(candidate, false, to)
    if (session && session.destroyed) throw session.error
    if (this.destroyed) throw REQUEST_DESTROYED()
    if (this.suspended || this._stateTransition !== null) throw IO_SUSPENDED()
    ...
    if (session && !session._attach(req)) throw session.error
  ...
  createCandidate(destination, response) {
    return this._createCandidate(destination, this._identity(destination, response), response)
  }

  _createCandidate(destination, snapshot, response) {
    const id = b4a.from(snapshot.id)
    if (!b4a.isBuffer(id) || id.byteLength !== 32) {
      throw response ? TRANSPORT_INVALID_RESPONSE() : TRANSPORT_INVALID()
    }
    const candidate = Object.freeze({ destination, key: snapshot.key, id })
    this._candidates.set(candidate, {
      destination,
      key: snapshot.key,
      id: b4a.from(id)
    })
  ...
  _candidateIdentity(candidate, response, destination = null) {
    const identity = this._candidates.get(candidate)
    if (identity === undefined || (destination !== null && identity.destination !== destination)) {
      throw response ? TRANSPORT_INVALID_RESPONSE() : TRANSPORT_INVALID()
    }
    return identity
  }

  takeReplyCandidates(reply) {
    const candidates = this._replyCandidates.get(reply)
    if (candidates === undefined) throw TRANSPORT_INVALID_RESPONSE()
    this._replyCandidates.delete(reply)
    return candidates
  }

  discardReplyCandidates(reply) {
    this._replyCandidates.delete(reply)
  }
  ...
  _validateReply(reply, retained) {
    ...
    let closerCandidates = null
    try {
      fromCandidate = admit(from)
      if (closerNodes !== null) {
        closerCandidates = new Array(closerNodes.length)
        for (let i = 0; i < closerNodes.length; i++) {
          closerCandidates[i] = admit(closerNodes[i])
        }
      }
    } finally {
      registry.clear()
    ...
    this._replyCandidates.set(normalized, {
      from: fromCandidate,
      closerNodes: closerCandidates
    })
    return normalized
    ...
    function admit(destination) {
      ...
      add(identity)
      return io._createCandidate(destination, identity, true)
    }
  }
...
class TransportRequest {
  constructor(io, to, identity, token, internal, command, target, value, session, context) {
    this.to = to
    this.token = token
    ...
    Object.defineProperty(this, '_transportContext', { value: context })
    this.index = -1
    this.sent = 0
    this.retries = 3
    this.destroyed = false
  ...
  _start(generation, attempt) {
    ...
    try {
      operation = this._io.adapter.request({
        ...
        target: this.target,
        value: this.value,
        context: this._transportContext,
        attempt
      })
    } catch (error) {
    ...
    const terminal = this.destroyed || generation !== this._generation
    const fields = operationFields(operation)
    if (terminal || this.destroyed || generation !== this._generation) {
      if (fields !== null) {
        fields.promise.then(noop, noop)
        this._cancelReturnedOperation(operation, fields.cancel, this._terminal)
      }
      return
    }
    if (fields === null) {
  ...
  _cancelReturnedOperation(operation, cancel, reason) {
    try {
      cancel.call(operation, reason || REQUEST_DESTROYED())
    } catch (error) {
