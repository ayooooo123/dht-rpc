...
module.exports = class Session {
  ...
  query({ target, command, value }, opts = {}) {
    ...
    const forwarded = sessionOptions(this.dht, opts, this, true)
    if (this.destroyed) throw this.error
    return this.dht.query({ target, command, value }, forwarded)
  }
  ...
  request({ token, command, target, value }, to, opts = {}) {
    ...
    const forwarded = sessionOptions(this.dht, opts, this, true)
    if (this.destroyed) return Promise.reject(this.error)
    return this.dht.request({ token, command, target, value }, to, forwarded)
  }

  ping(to, opts = {}) {
    if (this.destroyed) return Promise.reject(this.error)
    const forwarded = sessionOptions(this.dht, opts, this, false)
    if (this.destroyed) return Promise.reject(this.error)
    return this.dht.ping(to, forwarded)
  }

  destroy(err) {
    // Direct root sessions are reusable cancellation groups for backwards compatibility.
    // Child and transport-only sessions are terminal so routed attempts cannot be resurrected.
    if (this._parent === null && this.dht.outboundPolicy === 'direct') {
      const error = err || REQUEST_DESTROYED()
      while (this.children.size > 0) this.children.values().next().value.destroy(error)
      while (this.inflight.length) this.inflight[0].destroy(error)
      return
    }
...
function sessionOptions(dht, opts, session, includeTransportContext) {
  ...
  if (session.destroyed) return copy
