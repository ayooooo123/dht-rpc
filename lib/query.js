    ...
    try {
      const nodes =
        this.dht.outboundPolicy === 'transport-only'
          ? ownDataOption(opts, 'nodes') || opts.closestNodes
          : opts.nodes || opts.closestNodes
      ...
      if (this.dht.outboundPolicy === 'transport-only') throw TRANSPORT_INVALID_RESPONSE()
      throw error
    }
...
function ownDataOption(opts, name) {
  let descriptor = null
  try {
    descriptor = getOwnPropertyDescriptor(opts, name)
  } catch {
    throw TRANSPORT_INVALID_RESPONSE()
  }
  if (descriptor === undefined) return undefined
  if (!hasOwn(descriptor, 'value')) throw TRANSPORT_INVALID_RESPONSE()
